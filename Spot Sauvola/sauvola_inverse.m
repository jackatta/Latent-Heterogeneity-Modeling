...
% INVERSE is for light signal on dark background

function output=sauvola_inverse(image, varargin)
% Initialization
...
deviation = abs(meanSquare - mean.^2).^0.5;

% Sauvola
R = max(deviation(:));
threshold = mean + (mean-1).* k .* (deviation / R-1) ;
